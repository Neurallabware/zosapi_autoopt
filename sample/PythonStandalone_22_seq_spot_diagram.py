...
class PythonStandaloneApplication(object):
    ...
    def __init__(self, path=None):
        ...
        if isInitialized:
            ...
            if path is None:
                # Default Zemax installation path
                default_path = r"C:\Program Files\ANSYS Inc\v242\Zemax OpticStudio"
                if os.path.exists(default_path):
                    isInitialized = ZOSAPI_NetHelper.ZOSAPI_Initializer.Initialize(default_path)
                    if isInitialized:
                        dir = ZOSAPI_NetHelper.ZOSAPI_Initializer.GetZemaxDirectory()
                    else:
                        raise PythonStandaloneApplication.InitializationException(f"Unable to initialize with default path: {default_path}")
                else:
                    raise PythonStandaloneApplication.InitializationException("Unable to locate Zemax OpticStudio. Please provide the installation path manually.")
            else:
                raise PythonStandaloneApplication.InitializationException(f"Unable to initialize with provided path: {path}")
...
if __name__ == '__main__':
    zos = PythonStandaloneApplication("C:\\Program Files\\ANSYS Inc\\v242\\Zemax OpticStudio")
    ...
    try:
        ...
        # Fallback to alternative API methods for different versions
        try:
            spot_setting.Fields.SetFieldNumber(0)
            spot_setting.Wavelengths.SetWavelengthNumber(0)
        except AttributeError:
            # Alternative method 2 - use direct assignment if available
            try:
                spot_setting.FieldNumber = 0  # 设置为第1个视场
                spot_setting.WavelengthNumber = 0  # 0表示所有波长
            except:
                pass  # 使用默认设置
    #! [e22s06_py]
