...
class PythonStandaloneApplication(object):
    ...
    def __init__(self, path=None):
        ...
        try:
            ...
            # If registry reading fails, use a common installation path
            NetHelper = r"C:\Program Files\Zemax OpticStudio\ZOS-API\Libraries\ZOSAPI_NetHelper.dll"
            if not os.path.exists(NetHelper):
                NetHelper = r"C:\Program Files\ANSYS Inc\v242\Zemax OpticStudio\ZOS-API\Libraries\ZOSAPI_NetHelper.dll"
...
if __name__ == '__main__':
    # Try different common installation paths
    possible_paths = [
        None,  # Try auto-detection first
    ...
    for path in possible_paths:
        try:
            zos = PythonStandaloneApplication(path)
            print(f"Successfully connected to OpticStudio at: {path if path else 'auto-detected path'}")
            break
        except Exception as e:
            if path is None:
                print(f"Auto-detection failed: {e}")
            else:
                print(f"Failed to connect with path {path}: {e}")
            continue
    
    if zos is None:
        print("Could not establish connection to OpticStudio. Please check installation path.")
    ...
    #! [e23s08_py]
    # Create separate ray fan analysis for each field
    ray_results_all = {}
    ray_analyses = {}  # Keep references to analyses
    for field_idx in range(1, max_num_field + 1):
        ...
        # Set field and wavelength using official API method
        ray_settings.Field.SetFieldNumber(field_idx)
        ray_settings.Wavelength.SetWavelengthNumber(1)  # Use primary wavelength

        ray.ApplyAndWaitForCompletion()
        ray_results_all[field_idx] = ray.GetResults()
        ray_analyses[field_idx] = ray  # Keep reference, don't close yet
    #! [e23s08_py]
    ...
    for field in range(1, max_num_field + 1):
        ...
        ray_results = ray_results_all[field]
        ds = ray_results.GetDataSeries(0)  # Get first data series
    ...
    # Close all analysis windows after we're done with them
    for field_idx in range(1, max_num_field + 1):
        ray_analyses[field_idx].Close()
