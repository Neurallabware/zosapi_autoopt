...
def plot_multifield_spots(zos_manager, analyzer, 
    ...
    # Calculate subplot layout for fields
    import math
    n_cols = min(3, num_selected_fields)
    n_rows = math.ceil(num_selected_fields / n_cols)
    ...
    for plot_idx, field_idx in enumerate(field_indices):
        ...
        for wave_idx in wave_indices:
            ...
            label = f'λ={wave_value:.1f}nm' if num_selected_waves > 1 else ''
    ...
    # Hide extra subplots
    if len(axes) > num_selected_fields:
        for i in range(num_selected_fields, len(axes)):
...
def plot_multifield_rayfan(zos_manager, analyzer, 
    ...
    for plot_idx, field_idx in enumerate(field_indices):
        ...
        for wave_plot_idx, wave_idx in enumerate(wave_indices):
            ...
            label_x = f'λ={wave_value:.1f}nm' if num_selected_waves > 1 else ''
            ...
            label_y = f'λ={wave_value:.1f}nm' if num_selected_waves > 1 else ''
...
def plot_comprehensive_analysis(zos_manager, analyzer, 
    ...
    for plot_idx, field_idx in enumerate(field_indices[:3]):  # Limit to 3 fields for layout
        ...
        for wave_plot_idx, wave_idx in enumerate(wave_indices):
            spot_data = analyzer.analyze_spot_diagram(field_index=field_idx, wavelength_index=wave_idx, ray_density=3)
            ...
            color = colors[wave_idx % len(colors)]
            label = f'λ={wave_value:.1f}nm' if len(wave_indices) > 1 else ''
